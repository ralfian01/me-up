...
class DBBuilder
{
     ...
     */
    public function __construct($tableName, DBConnectionInterface $db, ?array $options = null)
    {
        if (empty($tableName)) {
            throw new BadMethodCallException('A table must be specified when creating a new Query Builder.');
        }
    ...
    //  */
    // public function selectSubquery(DBBuilder $subquery, string $as): self
    // {
     ...
     * @return string
     */
    protected function createAliasFromTable(string $item)
    {
    ...
    // /**
    //  * @param DBBuilder $from Expected subquery
    //  * @param string $alias Subquery alias 
    //  * @return $this
    //  */
    // public function fromSubquery(DBBuilder $from, string $alias): self
    // {
    ...
    /**
     * Generates the JOIN portion of the  
     ...
     * @param array|DBBuilder|Closure|string $values The values searched on, or anonymous function with subquery 
     * @return $this
     */
    ...
    /**
     * @param array|DBBuilder|Closure|null $values The values searched on, or anonymous function with subquery 
     * @return $this
     */
     ...
     * @return string
     */
    protected function groupGetType(string $type)
    {
     ...
     * @return string
     */
    protected function _limit(string $sql, bool $offsetIgnore = false)
    {
     ...
     * @return string
     */
    protected function compileFinalQuery(string $sql)
    {
     ...
     * @return false|int|string[] Number of affected rows or FALSE on failure
     */
     ...
     * @return string
     */
    protected function _upsertBatch(string $table, array $keys, array $values)
    {
     ...
     * @param DBBuilder|RawSql $query
     * @param array|string|null $columns an array or comma delimited string of columns
     * @return DBBuilder
     */
    public function setQueryAsData($query, ?string $alias = null, $columns = null)
    {
        if (is_string($query)) {
            throw new InvalidArgumentException('$query parameter must be DBBuilder or RawSql class.');
        }

        if ($query instanceof DBBuilder) {
     ...
     * @return string
     */
    protected function _insertBatch(string $table, array $keys, array $values)
    {
        $sql = $this->QBOptions['sql'] ?? '';
     ...
     * @return string
     */
    protected function removeAlias(string $from)
    {
     ...
     * @return string
     */
    protected function _insert(string $table, array $keys, array $unescapedKeys)
    {
     ...
     * @return string
     */
    protected function _replace(string $table, array $keys, array $values)
    {
     ...
     * @return string
     */
    protected function _update(string $table, array $values)
    {
        $valStr = [];
     ...
     * @return string
     */
    protected function _truncate(string $table)
    {
     ...
     * @return string
     */
    public function getCompiledDelete(bool $reset = true)
    {
     ...
     * @return string
     */
    protected function _deleteBatch(string $table, array $keys, array $values)
    {
        $sql = $this->QBOptions['sql'] ?? '';
     ...
     * @return string
     */
    protected function compileSelect($selectOverride = false)
    {
     ...
     * @return string
     */
    protected function compileWhereHaving(string $qbKey)
    {
        if (!empty($this->{$qbKey})) {
     ...
     * @return string
     */
    protected function unionInjection(string $sql)
    {
        if ($this->QBUnion === []) {
            return $sql;
        }
     ...
     * @return bool
     */
    protected function isLiteral(string $str)
    {
        $str = trim($str);
     ...
     * @return bool
     */
    protected function hasOperator(string $str)
    {
        return preg_match(
    ...
    {
        return $value instanceof DBBuilder || $value instanceof Closure;
    }

    /**
     * @param DBBuilder|Closure $builder
